...
from .includes import PositionManager, Side, Trade
...
class ResultReport:
    ...
    def generate_report(self) -> Dict[str, Dict[str, float]]:
        ...
        for ticker, increments in self.equity_curve.items():
            ...
            drawdown = np.minimum(pnl_cumsum, 0.0)
            max_dd = (drawdown.min() / self.initial_capital) * 100.0

            # --- Calmar ratio ---
            calmar = 0.0
            if max_dd > 0:
                calmar = pnl_pct / (max_dd * 100.0)  # pnl% / drawdown%
